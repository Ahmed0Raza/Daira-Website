import { useEffect } from 'react';
import { Navigate, Outlet } from 'react-router-dom';
import { useSnackbar } from '../../../utils/snackbarContextProvider';
import CreateAxiosInstance from '../../../utils/axiosInstance';
import { useSocietyAuth } from './societyAuth';
const societyRoute = '/coolboi69';

const SocietyProtectedRoute = () => {
  const userData = JSON.parse(localStorage.getItem('isSocietyData'));
  const { isSociety, setLoading, setIsSociety } = useSocietyAuth();
  const { show } = useSnackbar();
  const axios = CreateAxiosInstance();

  useEffect(() => {
    const verifyToken = async () => {
      setLoading(true);
      if (userData) {
        try {
          const res = await axios.get('/backend/societies/verify', {
            headers: {
              authorization: userData.result,
            },
          });
          if (res.status === 200) {
            setIsSociety(true);
          } else {
            throw new Error('Token validation failed');
          }
        } catch (error) {
          console.error('User verification error:', error);
          setIsSociety(false);
          localStorage.removeItem('isSocietyData');
          show('Authentication failed, please log in again', 'error');
        }
      } else {
        setIsSociety(false);
      }
      setLoading(false);
    };

    verifyToken();
  }, []);

  return isSociety ? <Outlet /> : <Navigate to={`${societyRoute}/login`} />;
};

export default SocietyProtectedRoute;
